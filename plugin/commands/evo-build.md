...
- **工作量（workload）**：benchmark 数据集的构建划分（construction split）问题。按
  `plugin/skills/build-semantic-layer/references/semantic-layer-data-boundary.md` 的
  fold 划分与 70/30 构建/验证切分，Builder 只能读构建集，**不得读取留出（held-out）的
  验证/测试问题及其 ground truth**。

## 执行

执行 `build-semantic-layer` skill（`plugin/skills/build-semantic-layer/`），构建初始语义层。
...
发布前调用 `plugin/scripts/validate.py --root <workspace>` 做确定性门禁（引用完整性 +
可加载）。
